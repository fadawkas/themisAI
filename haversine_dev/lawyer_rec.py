# lawyer_rec.py  (standalone version)

import os
import json
from math import radians, sin, cos, sqrt, atan2
from pathlib import Path
from typing import List, Dict, Any

import faiss
import numpy as np
from sentence_transformers import SentenceTransformer
import requests


# ============================
# Nominatim Geocode (langsung)
# ============================

NOMINATIM_BASE = "https://nominatim.openstreetmap.org"
SESSION = requests.Session()
HEADERS = {
    "User-Agent": "ppkhi-lawyer-recommender/1.0 (oemarkid@gmail.com)",
}

def geocode_user_location(address: str):
    """
    Geocode lokasi user menggunakan Nominatim.
    Mengembalikan lat/lon dalam float.
    """
    params = {
        "q": address,
        "format": "jsonv2",
        "limit": 1,
        "countrycodes": "id",
    }

    try:
        resp = SESSION.get(
            f"{NOMINATIM_BASE}/search",
            params=params,
            headers=HEADERS,
            timeout=10,
        )
        resp.raise_for_status()
        data = resp.json()
        if not data:
            return None
        first = data[0]
        return {
            "lat": float(first["lat"]),
            "lon": float(first["lon"]),
            "display_name": first.get("display_name"),
        }
    except Exception as e:
        print("Geocode error:", e)
        return None



# ============================
# CONFIG
# ============================

BASE_DIR = Path(__file__).resolve().parent

LAWYER_INDEX_DIR = BASE_DIR / "lawyers"

LAWYER_INDEX_PATH = LAWYER_INDEX_DIR / "index_lawyers.faiss"
LAWYER_META_PATH  = LAWYER_INDEX_DIR / "lawyers_meta.jsonl"

EMBED_MODEL = os.getenv(
    "EMBED_MODEL",
    "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2",
)

ALPHA_SEMANTIC = float(os.getenv("LAWYER_ALPHA_SEMANTIC", "0.6"))


# ============================
# LOAD INDEX & METADATA
# ============================

if not LAWYER_INDEX_PATH.exists():
    raise RuntimeError(f"Index not found: {LAWYER_INDEX_PATH}")

if not LAWYER_META_PATH.exists():
    raise RuntimeError(f"Metadata not found: {LAWYER_META_PATH}")

_index = faiss.read_index(str(LAWYER_INDEX_PATH))
_encoder = SentenceTransformer(EMBED_MODEL)

_lawyers = []
with LAWYER_META_PATH.open("r", encoding="utf-8") as f:
    for line in f:
        _lawyers.append(json.loads(line))

# Ambil lat/lon pengacara
_lawyer_latlon = []
for rec in _lawyers:
    lat = rec.get("latitude")
    lon = rec.get("longitude")
    if lat is None or lon is None:
        _lawyer_latlon.append((None, None))
    else:
        _lawyer_latlon.append((float(lat), float(lon)))

_lawyer_latlon = np.array(_lawyer_latlon, dtype=object)



# ============================
# Helpers
# ============================

def haversine_km(lat1, lon1, lat2, lon2):
    R = 6371.0
    dlat = radians(lat2 - lat1)
    dlon = radians(lon2 - lon1)
    a = sin(dlat/2)**2 + cos(radians(lat1))*cos(radians(lat2))*sin(dlon/2)**2
    c = 2 * atan2(sqrt(a), sqrt(1-a))
    return R * c


def embed_case(text: str):
    emb = _encoder.encode([text], convert_to_numpy=True)
    faiss.normalize_L2(emb)
    return emb.astype("float32")


def _semantic_search(query: str, top_k=50):
    q = embed_case(query)
    D, I = _index.search(q, top_k)
    return I[0], D[0]



# ============================
# Main Rekomendasi
# ============================

def recommend_lawyers(user_location, case_description, top_k=5, search_pool_k=50):

    # 1) Geocode lokasi user
    geo = geocode_user_location(user_location)
    if not geo:
        return {"error": "Failed to geocode location"}

    user_lat = geo["lat"]
    user_lon = geo["lon"]

    # 2) Semantic ranking
    idxs, sims = _semantic_search(case_description, top_k=search_pool_k)

    # 3) Combine semantic + distance
    results = []
    for idx, sim in zip(idxs, sims):
        if idx < 0:
            continue
        rec = _lawyers[idx]
        lawyer_lat, lawyer_lon = _lawyer_latlon[idx]

        semantic_score = (float(sim) + 1) / 2  # [-1,1] → [0,1]

        if lawyer_lat is None:
            distance_km = None
            distance_score = 0
        else:
            distance_km = haversine_km(user_lat, user_lon, lawyer_lat, lawyer_lon)
            distance_score = 1 / (1 + distance_km)

        final_score = (
            ALPHA_SEMANTIC * semantic_score +
            (1 - ALPHA_SEMANTIC) * distance_score
        )

        results.append({
            "name": rec.get("name"),
            "alamat_kantor": rec.get("alamat_kantor") or rec.get("alamat"),
            "cd cialitas": rec.get("specialitas"),
            "latitude": lawyer_lat,
            "longitude": lawyer_lon,
            "distance_km": distance_km,
            "semantic_score": semantic_score,
            "final_score": final_score,
        })

    results.sort(key=lambda x: x["final_score"], reverse=True)
    return results[:top_k]


if __name__ == "__main__":
    location = "Palmerah, Jakarta Barat"
    case = "Saya butuh pengacara untuk menangani kasus perlindungan anak."

    out = recommend_lawyers(location, case, top_k=5)
    print(json.dumps(out, indent=2, ensure_ascii=False))
